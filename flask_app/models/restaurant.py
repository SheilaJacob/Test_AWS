...
#the class method below is one restaurant where you can all the burgers which ties back to line 12 where it says self.burger =[], the list below is where it comes from 
    @classmethod
    def get_restaurant_with_burgers( cls , data ):
        query = "SELECT * FROM restaurants LEFT JOIN burgers ON restaurants.id0 = burgers.restaurant_id  WHERE restaurants.id = %(id)s;"
        ...
        for row_from_db in results:
            ...
            burger_data = {
                "id" : row_from_db["burgers.id"],           #Common columns that exists on the table will come back from in our results with the table name attached.thats why where calling it burgers.id instead of id alone
